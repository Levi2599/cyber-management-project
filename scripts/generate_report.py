...
def build():
    ...
    para(doc,
         "הסוכן מדמה תוקף חיצוני. ה-System Prompt מנחה אותו לאתר חולשות, לייצר PoC, ולחשב מדדי "
    ...
    para(doc,
         ...
         "ומחייבת שהממצא יהיה מנוסח בצורה שניתן לפעול לפיה.")
    para(doc,
         "חשוב לציין שהארכיטקטורה הזו אינה מבטלת התראות שווא. אם הסוכן האדום מדווח על חולשה שאינה "
         "קיימת, הסוכן הכחול יקבל את הדיווח כנתון וינסה לתקן. השיפור הצפוי נובע מהאילוץ שהממצא "
         "חייב להיות ספציפי מספיק כדי לייצר ממנו תיקון, לא ממנגנון הצבעה או ביקורת הדדית. "
         "כימות ההשפעה בפועל דורש הרצת ההערכה.")
    ...
    para(doc,
         "הפרויקט מדגים שניתן לבנות צינור אוטומטי שמחבר אירוע Pull Request לניתוח אבטחה ולהצעת תיקון, "
