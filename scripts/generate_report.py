...
def build():
    ...
    if results:
        ...
        # ניתוח ההתראות שווא נגזר מהרשומות עצמן, לא נכתב ידנית.
        fps = [r for r in results["records"]
               if r["label"] == "safe" and r.get("predicted_vulnerable")]
        by_id = {c["id"]: c for c in cases}

        heading(doc, "ניתוח התראות השווא", Pt(12), md_level=3, space_before=Pt(6))
        if fps:
            para(doc,
                 "המערכת זיהתה את כל %d המקרים הפגיעים, אך סימנה גם %d מתוך %d המקרים הבטוחים "
                 "כפגיעים. זהו הממצא המרכזי של ההערכה, והוא מנוגד לציפייה שהארכיטקטורה "
                 "הדו-סוכנית תקטין את שיעור התראות השווא."
                 % (s["true_positives"], s["false_positives"], s["safe_cases"]))

            rows = []
            for r in fps:
                c = by_id.get(r["id"], {})
                rows.append([r["id"], c.get("language", "—"),
                             r.get("cvss_vector") or "—"])
            table(doc, ["מזהה", "שפה", "וקטור CVSS שדווח"], rows)

            para(doc,
                 "בחינה של שלושת המקרים מלמדת שכולם הם הגרסה **המתוקנת** של חולשה מקבילה "
                 "בקורפוס: תבנית תצוגה בצד השרת במקום שרשור מחרוזות, בדיקת נתיב מוחלט מול "
                 "תיקיית בסיס, ופענוח JSON עם אימות טיפוס. כלומר הסוכן האדום מזהה נכון את "
                 "*אזור* הסיכון, אך אינו מזהה שאמצעי ההגנה שבמקום כבר סוגר אותו. "
                 "הוא מדווח על הדפוס ולא על הפגיעות בפועל.")
            para(doc,
                 "ההסבר הזה גם מסביר את קצב הזיהוי המושלם: סוכן שנוטה לסמן כל דפוס חשוד "
                 "לא יחמיץ חולשה אמיתית, אבל ישלם על כך בהתראות שווא. שני המדדים הם שני צדדים "
                 "של אותה הטיה, ולכן אין לקרוא את קצב הזיהוי בנפרד מהם.")
    ...
    if results:
        para(doc,
             "המדידה מאשרת את המגבלה הזו במלואה. שיעור התראות השווא שנמדד עומד על %s%%, "
             "כלומר הארכיטקטורה הדו-סוכנית לא סיננה אותן. הציפייה שהפרדת התפקידים תקטין "
             "מעצמה את רעש ההתראות לא התממשה, ומי שמתכנן מערכת כזו אינו יכול להסתמך "
             "על ההפרדה ככלי סינון."
